...
const Post: React.FC<Props> = ({ post }) => {
  ...
  const isLiked = false; // This can be determined based on your logic
  const isMyPost = true; // This can be determined based on your logic
  const formattedDate = "1h"; // You can dynamically calculate the date
        ...
        <div className="flex justify-between mt-3">
          <div className="flex gap-4 items-center w-2/3 justify-between">
            <div
              ...
              onClick={() => {
                const dialog = document.getElementById(`comments_modal${post._id}`) as HTMLDialogElement;
                dialog?.showModal();
              }}
            >
            ...
            <dialog id={`comments_modal${post._id}`} className="modal border-none outline-none">
              ...
              <form method="dialog" className="modal-backdrop">
                <button type="button" className="outline-none" onClick={() => {
                  const dialog = document.getElementById(`comments_modal${post._id}`) as HTMLDialogElement;
                  dialog?.close();
                }}>
                  Close
